...
class Map( object ):
	...
	def __init__( self, grid_size, *args, **keywords ):
		#Map size
		( rows, cols ) = grid_size
		self.rows = rows
		self.cols = cols
...
#				unit = "U" if units and self.positions.get( ( row + col / 2, col ) ) else ""
	...
	def neighbors( self, center ):
		...
		"""
		return list(filter( self.valid_cell, [
	...
	def spread( self, center, radius=1 ):
		...
		return list(filter( self.valid_cell, result ))#filter invalid cells before returning.
	...
	def cone( self, origin, direction, length=1 ):
		...
		return list(filter( self.valid_cell, set( result ) ))
	...
	def slice( self, origin, direction, length=2 ):
		...
		for i in range( 1, length + 1 ):
			...
			for j in range( i + 1 ):
				...
				result.append( pos )
		return list(filter( self.valid_cell, result ))
	...
	def line( self, origin, direction, length=3 ):
		...
		return list(filter( self.valid_cell, results ))
	...
	def cells( self ):
		...
		return list(filter( self.valid_cell, cells ))
...
class Grid( dict ):
	...
	def find( self, item ):
		...
		"""
		for pos, value in list(self.items()):
			if item == value:
				return pos
		return None

class MapUnit( object, metaclass=ABCMeta ):
	"""
